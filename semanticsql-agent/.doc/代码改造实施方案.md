...
pip install jinja2==3.1.2  # 用于提示词模板
```
...
python -c "from utils.database import DatabaseManager; 
          ...
          print('数据库连接正常' if db.initialize() else '数据库连接失败')"
```
...
- 所有分析结果存储在记忆中，供后续工具使用

## 反思机制
当 sql_reflection 发现问题时：
- problem_source 指示问题来源
- recommended_action 建议下一步调用的工具
- 你需要根据建议采取相应行动
```
